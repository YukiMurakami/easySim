...
while [ $a -ne 32 ]
do
scp ${DIRECTORY}"0214-1/"${PARA}${PLACE[a]}${CP}".txt" ./${PLAYOUT}
a=`expr $a + 1`
done
...
if [ $a -lt 8 ]
then
    scp ${DIRECTORY}"0214-2/"${PARA}${PLACE[a]}${CP}".txt" ./${PLAYOUT}
else
    if [ $a -lt 16 ]
    then
        scp ${DIRECTORY}"0214-3/"${PARA}${PLACE[a]}${CP}".txt" ./${PLAYOUT}
    else
        if [ $a -lt 24 ]
        then
            scp ${DIRECTORY}"0214-4/"${PARA}${PLACE[a]}${CP}".txt" ./${PLAYOUT}
        else
            scp ${DIRECTORY}"0214-5/"${PARA}${PLACE[a]}${CP}".txt" ./${PLAYOUT}
        fi
    fi
fi
a=`expr $a + 1`
